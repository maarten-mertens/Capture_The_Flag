...
```
To revert de hex file do: 
```bash
bandit12@bandit:/tmp/twatwa$ xxd -r data.txt data.bin
```
Hierna is het nodig op met behulp van de verschillende compressie en archiveringstools een bestand uit te pakken.

bepaal het filetype en voer het bijhorende commando uit:
```
...
het laatste bestand is een tekstbestand met de flag.
